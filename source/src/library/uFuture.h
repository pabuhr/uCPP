...
namespace UPP {
	...
	void check() {
	    if ( cancelled() ) _Throw uCancelled();
	    if ( cause != nullptr ) cause->reraise();	// deliver inserted exception
	} // uBaseFuture::check
      public:
	...
	void exception( uBaseEvent * ex ) {		// make exception available in the future : exception and result mutual exclusive
	    if ( available() || cancelled() ) _Throw uDelivered(); // already set or client does not want it
	    cause = ex;
	    makeavailable();				// unblock waiting clients ?
	} // uBaseFuture::exception

	void delivery( T res ) {			// make result available in the future
	    if ( cancelled() || available() ) _Throw uDelivered(); // already set or client does not want it
	    result = res;
	    makeavailable();
	} // uBaseFuture::delivery

	void delivery( uBaseEvent * ex ) {		// alternate interface
	    exception( ex );
	} // uBaseFuture::delivery
...
template<typename T> class Future_ISM {
  ...
  public:
    ...
    void delivery( uBaseEvent * cause ) {		// alternate interface
	impl->exception( cause );
    } // Future_ISM::delivery
...
//	    if ( buf.empty() ) delay.wait();		// no request to process ? => wait
	...
	void main() {
		...
		ELEMTYPE * request = requests[i + start].remove();
	      if ( ! request ) {
