...
namespace UPP {
    ...
    // a a false _When condition, it returns GuardFalse.  It is up to the parent node to interpret GuardFalse.  Note
    // there are only three possibilities.  If the parent node is an "and" binary node, GuardFalse is interpreted as
    // Avail, i.e., that branch has an available future, so the other branch still has to become available.  If the
    // parent node is an "or" binary node, GuardFalse is interpreted as NonAvail, i.e., that branch has an unavailable
    // future, so the other branch still has to become available. GuardFalse on both branches means that branch is
    // turned off, and is interpreted as Avail.
