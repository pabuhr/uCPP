%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -*- Mode: Latex -*- %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 
%% uC++ Version 6.0.0, Copyright (C) Peter A. Buhr
%% 
...
%% Last Modified On : Sat Dec 29 20:49:19 2012
%% Update Count     : 6712
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vspace{0.125in}
\href{http://plg.uwaterloo.ca/~usystem/pub/uSystem/u++-6.0.0.sh}{\uC download} (installation: \textsf{sudo sh u++-6.0.0.sh})
} % title
\author{
...
\L{\LB{}\Tab{8}{\V{cout}\0\<\<\0\S{}\"prod\0payment\0of\0\$\"\SE{}\0\<\<}}
\L{\LB{}\Tab{12}{\V{money}\0\<\<\0\V{endl};}}
\L{\LB{}\Tab{8}{\V{resume}();}\Tab{24}{\C{}\1\1\0restart\0prod}}
\CE{}\L{\LB{}\Tab{8}{\V{receipt}\0+=\01;}\Tab{24}{\C{}\1\1\0in\0Cons::delivery}}
...
\L{\LB{\0\0\K{public}:}}
\L{\LB{}\Tab{4}{\V{uSemaphore}(\0\K{unsigned}\0\K{int}\0\V{count}\0=\01\0);}}
...
\L{\LB{}\Tab{4}{\K{void}\0\V{V}(\0\K{unsigned}\0\K{int}\0\V{times}\0=\01\0);}}
...
Such a call is known as a \newterm{\Index{synchronous call}} because the two tasks must synchronize in order to pass the arguments from caller to callee, and because the caller remains blocked until the callee returns the result.
...
All exception types are also grouped into a hierarchy, where the hierarchy is built by publicly inheriting among the exception types.
...
Unfortunately, resumption handlers must be simulated using routines/functors due to the lack of nested routines in \CC.
\item
...
While \CC allows any type to be used as an exception type, \uC restricts exception to types defined by \LGinlinetrue\LGbegin\lgrinde\L{\LB{\K{\_Event}}}\endlgrinde\LGend{}\index{_Event@\LGinlinetrue\LGbegin\lgrinde\L{\LB{\K{\_Event}}}\endlgrinde\LGend{}}\index{keyword, additions!_Event@\LGinlinetrue\LGbegin\lgrinde\L{\LB{\K{\_Event}}}\endlgrinde\LGend{}}.
An exception type\index{exception!type} has all the properties of a \LGinlinetrue\LGbegin\lgrinde\L{\LB{\K{class}}}\endlgrinde\LGend{}, and its general form is:
\LGinlinefalse\LGbegin\lgrinde
...
The\label{p:ListPollingPoints} \uC kernel polls implicitly for nonlocal exceptions (and cancellation, see \VRef{s:Cancellation}) when the following occur:
...
Furthermore, in the multiprocessor case, there are multiple kernel threads that are not necessarily handled well by all debuggers.
Some debuggers do handle multiple kernel threads (which correspond to \uC virtual processors), and hence, it is possible to examine at least the active tasks running on each kernel thread.
Nevertheless, it is possible to use many debuggers on programs compiled with the unikernel.
At the very least, it is usually possible to examine some of the variables, externals and ones local to the current coroutine or task, and to discover the statement where the error occurred.

For most debuggers it is necessary to tell them to let the \uC runtime system handle certain UNIX signals.
Signals \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{SIGALRM}}}\endlgrinde\LGend{} and \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{SIGUSR1}}}\endlgrinde\LGend{} are handled by \uC to perform pre-emptive scheduling\index{pre-emptive!scheduling}.
In gdb, the following debugger command allows the application program to handle signal \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{SIGALRM}}}\endlgrinde\LGend{} and \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{SIGUSR1}}}\endlgrinde\LGend{}:
\LGinlinefalse\LGbegin\lgrinde
\L{\LB{\V{handle}\0\V{SIGALRM}\0\V{nostop}\0\V{noprint}\0\V{pass}\0\V{ignore}}}
\L{\LB{\V{handle}\0\V{SIGUSR1}\0\V{nostop}\0\V{noprint}\0\V{pass}\0\V{ignore}}}
\endlgrinde\LGend
...
The current version of \uC can be obtained from:
\begin{quote}
\href{http://plg.uwaterloo.ca/~usystem/pub/uSystem/u++-6.0.0.sh}{\textsf{http:\,//\hspace{0.1ex}plg.uwaterloo.ca\,/$\sim$usystem/pub/uSystem/u++-6.0.0.sh}}
\end{quote}
Execute the following command to install:
\LGinlinefalse\LGbegin\lgrinde
\L{\LB{\%\0\V{sudo}\0\K{\#}\0\V{optional},\0\V{installs}\0\V{in}\0\1\V{usr}\1\V{local}\0\K{and}\0\1\V{usr}\1\V{local}\1\V{bin}}}
\L{\LB{\%\0\V{sh}\0\V{u}++\-6.0.0.sh}}
\endlgrinde\LGend
