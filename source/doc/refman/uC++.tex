...
%% Last Modified On : Tue Jul 19 08:47:15 2022
%% Update Count     : 8430
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In general, a client actor creates a \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{PromiseMsg}}}\endlgrinde\LGend{} containing data for the server to process and an empty \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{Promise}}}\endlgrinde\LGend{}, which it sends to a server actor.
...
If the client wins, it sees the installed callback during result \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{deliver}}}\endlgrinde\LGend{}, and the server posts (special message send) the callback function onto the mailbox queue for the client's executor.
...
is the same as \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{maybe}}}\endlgrinde\LGend{}, except that the callback passed is called unconditionally once the promise has been fulfilled.
...
Hence, the callback passed to \LGinlinetrue\LGbegin\lgrinde\L{\LB{\V{then}}}\endlgrinde\LGend{} is always called, but may either be installed or immediately called.
...
\CE{}\L{\LB{\V{uint32\_t}\0\BF{\V{uBaseTask}::}\V{prng}(\0\V{uint32\_t}\0\V{u}\0);\0\C{}\1\1\0[0,u)}}
\CE{}\L{\LB{\V{uint32\_t}\0\BF{\V{uBaseTask}::}\V{prng}(\0\V{uint32\_t}\0\V{l},\0\V{uint32\_t}\0\V{u}\0);\0\C{}\1\1\0[l,u]}}
\CE{}\endlgrinde\LGend
